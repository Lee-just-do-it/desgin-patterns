...
public abstract class AbstractBuilder<T> {
    ...
    private AbstractPayTemplate template = null;

    protected abstract void set();

    protected <T extends AbstractPayTemplate> T build(Class<T> c) {
        try {
            template = (T) Class.forName(c.getName()).newInstance();
        } catch (Exception e) {
    ...
    protected AbstractPayTemplate build() {
        return template;
    }
}
